...
const THEME_DIR: &'static str = "src/schemes/";

// Dot file constants
const THEMED_DIR: &'static str = "dotfiles/themed/";
const UNTHEMED_DIR: &'static str = "dotfiles/unthemed/";

const XRES: &'static str = ".Xresources";
const I3CONFIG: &'static str = "config";
const I3STATUS: &'static str = "i3status.conf";
const VIM: &'static str = "coordinated.vim";
const DMENU: &'static str = "dmenu_run_themed";

const XRES_FINAL_DEST: &'static str = "/home/hunter/";
const I3CONFIG_FINAL_DEST: &'static str = "/home/hunter/.i3/";
const I3STATUS_FINAL_DEST: &'static str = "/home/hunter/.i3/";
const VIM_FINAL_DEST: &'static str = "/home/hunter/.vim/colors/";
const DMENU_FINAL_DEST: &'static str = "/home/hunter/.i3/";
...
fn generate_vim_theme(c: &ColorScheme) -> io::Result<()> {
    println!("Generating: {}", VIM);

    let mut themed_f = try!(File::create(get_themed_path(&VIM)));

    let unthemed_f = match File::open(Path::new(&get_unthemed_path(&VIM))) {
...
fn generate_dmenu_script(c: &ColorScheme) -> io::Result<()> {
    println!("Generating: {}", DMENU);

    let mut themed_f = try!(File::create(get_themed_path(&DMENU)));
...
fn generate_themed_i3status(c: &ColorScheme) -> io::Result<()> {
    println!("Generating: {}", &I3STATUS);

    let mut themed_f = try!(File::create(get_themed_path(&I3STATUS)));
    ...
    let unthemed_f = match File::open(Path::new(&get_unthemed_path(&I3STATUS))) {
        Ok(unthemed_f) => unthemed_f,
...
fn get_themed_path(f: &str) -> String {
    THEMED_DIR.to_string() + f
}

fn get_unthemed_path(f: &str) -> String {
    UNTHEMED_DIR.to_string() + f
}
...
fn list_available_themes() -> io::Result<()> {
    ...
    for entry in try!(fs::read_dir(THEME_DIR)) {
        let file = try!(entry);
        let path = file.path();
...
fn main() {
    ...
    generate_themed_file(&c, &XRES, xres_args)
        .ok()
        .expect("Couldn't write themed Xresource file!");

    let conf_args = ("set $", " ", "");
    generate_themed_file(&c, &I3CONFIG, conf_args)
        .ok()
